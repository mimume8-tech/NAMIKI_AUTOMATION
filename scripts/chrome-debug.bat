...
start "" "C:\Program Files\Google\Chrome\Application\chrome.exe" --remote-debugging-port=9222 --remote-debugging-address=127.0.0.1 --new-window --kiosk-printing --show-bookmark-bar --auto-select-certificate-for-urls={^"pattern^":^"*://digikar.jp^"} --user-data-dir="C:\ChromeDebugProfile" --profile-directory=Default https://digikar.jp/reception/
echo Chrome を起動しました。
